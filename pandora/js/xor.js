...
function rgbToHex(_r, _g, _b) {
    var r = parseInt((255 / 100) * parseInt(_r));
    var g = parseInt((255 / 100) * parseInt(_g));
    var b = parseInt((255 / 100) * parseInt(_b));
...
["FFFFFF", "White"]
    ...
    {
        var colour = '';
        jQuery.each(ntc.names, function(n)
        {
            if(ntc.names[n][1] == name) 
            {
                colour = '#' + ntc.names[n][0];
            }
        });
        return colour;
    },
    
    randomColour: function(seed)
    {
        var colours = ntc.names;
        var random = new XorShift128(parseInt(seed));
        var random_integer = random.integer(0, (parseInt(colours.length) - 1));
        return colours[random_integer][0];
    }
...
function stringToSeed(raw_label)
{
    ...
    var units = [];
    var counter = 0;
    // Convert label into path ...
    for(x = 0; x < label.length; x++)
    {
        var c = label.charAt(x);
