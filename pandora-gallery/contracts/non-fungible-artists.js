...
// TI (RECTANGLES) = 13450009027475933778071085108167758525301537160833508956033435178133434650793
// CA (LINES) = 46981312233805754808904953643039165448378674963225250914073902170976107857070
// XX (LINES) = 110333710129284309164548544842263444922480382591934151746513766057407273587629
// DO (CIRCLES) = 23475274943148462754599554450859676966268409252122676274227441331469612408765
// AN (CROSSES) = 52075855611873745092429898856551156405947085046839582042345667212604015201866
...
contract NonFungibleArtists is ERC721Pausable 
{
    ...
    */
    ...
    function getArtist(uint256 artist) public view returns
    (
        string memory firstInitial,
        string memory secondInitial,
        string memory style,
        uint blockNumber,
        address creator,
        address owner
        ...
        return(
            _artists[artist].first,
            _artists[artist].second,
            _artists[artist].style,
            _artists[artist].block,
            _artists[artist].creator,
            ownerOf(artist)
        );
    }
    ...
    {
        uint256 AID = 0;
        if(isCapital(firstInitial) && isCapital(secondInitial))
        {
            AID = uint256(keccak256(abi.encodePacked(artistAddress.toString(), '|', firstInitial, '|', secondInitial)));
        }
        return AID;
    }
    
    function getAttributes(uint256 artist) public view returns
    (
        uint bodyStyle,
        uint headStyle,
        uint eyeStyle,
        uint mouthStyle,
        uint accessoriesStyle,
        uint bodyAndHeadColour,
        uint eyeAndMouthColour,
        uint accessoriesColour
    ){
    ...
    */
    ...
    function _setArtisticStyle(uint256 artist) internal
    {
        uint256 style = random('style', 5, 1, artist);
        if(style == 2)
        {
            _artists[artist].style = "Rectangles";
        }
        else if(style == 3)
        {
            _artists[artist].style = "Lines";
        }
        else if(style == 4)
        {
            _artists[artist].style = "Circles";
        }
        else if(style == 5)
        {
            _artists[artist].style = "Crosses";
        }
        else 
        {
            _artists[artist].style = "Squares";
        }
    }
    ...
    function random(string memory seed, uint max, uint specificBlock, uint id) internal view returns(uint)
    {
        ...
        if(id > 0)
        {
            return uint256(keccak256(abi.encodePacked(seed, id))) % max + 1;
        }
        else
        {
