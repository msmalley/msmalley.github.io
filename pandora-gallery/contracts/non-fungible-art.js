...
// ABG (SQUARES) = 8280014841493775768269889089140861101248968995127777670513128502581481679673

// TIN (RECTANGLES) = 69984170702987381290309537176874370225028844312874816477667819560514941546731
// TIP (RECTANGLES) = 72433975199183629307991977809769806792884668132126573071342444737766792464911
// TIM (RECTANGLES) = 69544949948015069734918543146121324721257548822950280321460481177646092164697

// CAP (LINES) = 92534124047290146278043326703167710883639173503604198568420372031349076814846
// CAR (LINES) = 13365509954640446027089386226120574655443475842229233781010355947666665675637
// XXD (LINES) = 100812401244203211575870309660881945455283896459085784354670057419644854231137

// DOS (CIRCLES) = 47460250141908196842333047280368161694251853271219034994987677364717398098359
// DOA (CIRCLES) = 108509900992095891178012528350690267225311108381293536026076356923394241306042
// DOG (CIRCLES) = 24743269374905393971984838330046267805598164433783484730662098565250144866952

// ANT (CROSSES) = 2026021459919010537862043921991336462514764039207126140806084860050495268800
// ANY (CROSSES) = 61018971902072860888502452389289464420248782030659390721759027961316311643290
// ANX (CROSSES) = 113279331518578651873525886796810563361323253774480485509439805514992476220168
...
interface IERC721Metadata is IERC721 {
    ...
    function symbolBytes() external view returns (bytes32);
...
abstract contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    */
    ...
    function symbolBytes() public view virtual override returns (bytes32) {
        return _ToBytes32(_symbol);
    }
 ...
 */
abstract contract ERC721Burnable is ERC721URIStorage {
    /**
 ...
 */
abstract contract Pausable is ERC721Burnable {
    /**
 ...
 */
abstract contract ERC721Pausable is Pausable {
    /**
...
abstract contract NonFungibleArtists is ERC721Pausable 
{
    function getArtisticStyleBytes(uint256 artist) public virtual returns(bytes memory);
}

interface NonFungibleStyles
{
    function getOwner() external view returns(address);
    function generateArt(uint256 nft_id) external view returns(bytes memory);
}

contract NonFungibleArt is ERC721Pausable 
{
    NonFungibleArtists artists;
    NonFungibleStyles styles;
    ...
    {
        require(stylesAddress == address(0));
        styles = NonFungibleStyles(styleAddress);
        stylesAddress = styleAddress;
        require(styles.getOwner() == tx.origin);
    }
    ...
    */
    ...
    {   
        return string(styles.generateArt(nft_id));
    }
    
    function getArtist(uint256 nft_id) public view returns(uint256 artist)
    {
    ...
    function getArt(uint256 nft_id) public view returns
    (
    ...
    {
        return(
            _arts[nft_id].initial,
            _arts[nft_id].style,
            _arts[nft_id].block,
            _arts[nft_id].creator,
            ownerOf(nft_id),
            _arts[nft_id].artist
        );
    }
    ...
    */
    ...
    {
        uint256 AID = 0;
        if(isCapital(initial))
        {
            AID = uint256(keccak256(abi.encodePacked(creatorAddress.toString(), '|', artist.toString(), '|', initial)));
        }
        return AID;
    }
}